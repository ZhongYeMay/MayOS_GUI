...
class VersionManager:
    ...
    def get_current_version():
        """获取当前版本信息"""
        try:
            with open(f"MayOS_with_GUI_0.0.4_test/system/info/ver.txt", "r", encoding="utf-8") as f:
                return f.read().strip()
        except Exception:
            return "MayOS_GUI_0.0.3"  # 默认版本

    @staticmethod
    def get_version_content(version):
        """获取指定版本的内容"""
        try:
            with open(f"MayOS_with_GUI_0.0.4_test/system/info/MayOS_with_GUI_0.0.4_test.txt", "r", encoding="utf-8") as f:
                return f.read()
        except Exception:
    ...
    def initialize_version_files():
        """确保版本文件存在"""
        os.makedirs("MayOS_with_GUI_0.0.4_test/system/info", exist_ok=True)
        
        # 如果ver.txt不存在则创建
        if not os.path.exists("MayOS_with_GUI_0.0.4_test/system/info/ver.txt"):
            with open("MayOS_with_GUI_0.0.4_test/system/info/ver.txt", "w", encoding="utf-8") as f:
                f.write("MayOS_GUI_0.0.3")
...
def update_bing_wallpaper():
    ...
    try:
        ...
        # 确保背景目录存在
        os.makedirs("MayOS_with_GUI_0.0.4_test/files/background", exist_ok=True)

        # 保存图片
        today = datetime.now().strftime("%Y%m%d")
        save_path = f"MayOS_with_GUI_0.0.4_test/files/background/bing_{today}.jpg"
...
help_menu.add_command(label="官方网址", command=lambda: webbrowser.open("https://maydos-team.github.io"))
...
# 加载背景图片
try:
    bg_path = "MayOS_with_GUI_0.0.4_test/files/background/default.jpg"
