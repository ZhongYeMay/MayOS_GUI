...
try:
    ...
    if bg_photo:
        ...
        calc_btn.place(relx=0.1, rely=0.3, anchor=tk.CENTER)
        ...
        music_btn.place(relx=0.1, rely=0.4, anchor=tk.CENTER)
    else:
        # 加载默认壁纸
        default_path = os.path.join("files", "background", "default.jpg")
        if os.path.exists(default_path):
            default_img = Image.open(default_path).resize((1280, 720), Image.LANCZOS)
            default_photo = ImageTk.PhotoImage(default_img)
            test_label = Label(MayOSDesktop, image=default_photo)
            test_label.image = default_photo  # 保持引用防止垃圾回收
            test_label.pack(fill=tk.BOTH, expand=True)
